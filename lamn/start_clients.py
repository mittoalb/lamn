...
def start_client_on(ip, username, password, conda_env, screen_name, launch_cmd, auto_restart=False):
    print(f"[+] Connecting to {ip} to start client...")
    
    if auto_restart:
        # Modified script with auto-restart loop
        remote_script = f"""
            screen -dmS {screen_name} bash -c '
                while true; do
                    ...
                    {launch_cmd}
                    echo "$(date +"%H:%M:%S"): restart" >> /tmp/lamn_restart.log
                    sleep 30
                done
            '
        """
    else:
        # Original script - no auto-restart
        remote_script = f"""
            screen -dmS {screen_name} bash -c '
                source ~/.bashrc
                conda activate {conda_env}
...
def main():
    ...
    parser.add_argument('--auto-restart', action='store_true', help="Enable auto-restart if client dies")
    ...
    for ip in agents:
        try:
            start_client_on(ip, args.username, password, conda_env, screen_name, launch_cmd, args.auto_restart)
        except Exception as e:
