...
def start_client_on(ip, username, conda_env, screen_name, launch_cmd):
    ...
    proc = subprocess.Popen(
        ['ssh', f'{username}@{ip}', 'bash -s'],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True
    )
    out, err = proc.communicate(remote_script)

    print(out.strip())
    if err.strip():
        print(f"[!] Error on {ip}: {err.strip()}")
...
def main():
    ...
    for ip in agents:
        try:
            start_client_on(ip, args.username, conda_env, screen_name, launch_cmd)
